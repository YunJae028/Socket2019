...
int main(){
	...
	while(1){ //무한 루프
		...
		c=read(c_socket, rcvBuffer_c,sizeof(rcvBuffer_c));
		if(c<0){
		printf("Read Failed\n");
		return -1;
		}

		rcvBuffer_c[c] = '\0'; //문자열 뒷부분 깨짐 방지
		printf("received data: %s\n", rcvBuffer_c); //서버에서 받은 메세지 출력

		n = strlen(rcvBuffer_c);
		write(c_socket, rcvBuffer_c, n); //클라이언트에게 buffer의 내용을 전송함
